...
st.set_page_config(page_title="V51.0 CACHE DOMINANCE", layout="wide", page_icon="💎")
...
# 🛡️ FITUR KUNCI: CACHE UNTUK TRADINGVIEW (ANTI RATE-LIMIT)
# Data dibekukan selama 300 detik (5 menit). Berapapun tombol yang dipencet, TV tidak akan dipanggil lagi.
...
@st.cache_data(ttl=300, show_spinner=False)
def fetch_tv_sector_safely(ticker):
    try:
        q = Query().set_markets('indonesia').select('name','sector').where(Column('name') == ticker)
        _, df = q.get_scanner_data()
        if not df.empty: return df.iloc[0]['sector']
    except: pass
    return "IDX"

def run_deep_audit(ticker, ihsg_ret):
    try:
        time.sleep(random.uniform(0.2, 0.5)) # Sedikit delay agar Yahoo tidak mengamuk
...
# --- 🛰️ HEADER ---
st.markdown(f"<div class='status-card'><h1 style='margin:0; font-size: 28px; color:#ddd6fe;'>🏆 V51.0 CACHE DOMINANCE</h1><p style='margin:0; opacity:0.8;'>Engine: Frozen TV Cache (Anti Rate-Limit) + V8 YFinance ⚡</p></div>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if st.button("🔄 Segarkan Data (Bypass Cache)"):
        st.cache_data.clear()
        st.success("Data berhasil disegarkan dari server utama!")
...
if is_market_open or bypass:
    ...
    # ⚡ PANGGIL TRADINGVIEW (MENGGUNAKAN KULKAS/CACHE)
    df_raw, tv_online = fetch_tv_data_safely(max_p)
    valid_signals = []
    
    if not tv_online or df_raw.empty:
        st.warning("⚠️ TradingView membatasi akses (Rate Limit). Gunakan tombol 'Segarkan Data' dalam 3-5 menit ke depan.")
    else:
        if mode == "Turbo (Fast)":
            for _, row in df_raw.iterrows():
                valid_signals.append((row['name'], row['sector'], {"Lolos Likuiditas TV": True}, row['close']))
        else:
    ...
    if valid_signals:
        ...
        st.info("Radar selesai memindai. Belum ada sinyal kuat yang lolos filter.")
    ...
    if st.button("🚀 Run Tactical Audit"):
        if tid:
            with st.spinner(f"Membedah {tid}..."):
                # CEK SEKTOR VIA TV (JUGA DENGAN CACHE)
                sector_info = fetch_tv_sector_safely(tid)
                st.write(f"Sektor: **{sector_info}**")
                
                # INTEROGASI VIA YFINANCE
