...
st.markdown("""
    ...
    .pyramid-box { background-color: #1e1b4b; border: 1px dashed #6366f1; padding: 10px; border-radius: 8px; margin-top: 10px; font-size: 12px; }
...
mesin_aktif = True # Override for testing or set back to time logic

# --- 🌍 CORE ALGORITHM (THE GENERAL'S BRAIN) ---
...
def check_h1_interceptor(ticker):
    try:
        ...
        return h1_data['Volume'].iloc[-1] > (h1_data['Volume'].iloc[-5:-1].mean() * 1.5)
    except: return False

def run_audit(ticker, ihsg_ret):
    """Fungsi Kolom Cek Saham Manual"""
    try:
        ...
        checks = {
            "Minervini Template": c > sma150 and sma150 > sma200,
            "Above SMA 50": c > sma50,
            "Alpha Leader": stock_ret > ihsg_ret,
...
<div class='status-card bg-sector'>
    ...
    <div style='display: flex; justify-content: space-between; align-items: center;'>
        <p style='margin:5px 0 0 0; opacity:0.9; color:#a78bfa;'>Tactical Pyramiding | Manual Audit Mode | Ghost Detector</p>
...
# --- 🚀 LIVE SCAN ENGINE ---
if mesin_aktif:
    with st.status("Scanning Universe...", expanded=False) as status:
        ...
        for _, row in df_raw.iterrows():
            ...
            if audit_res and all(audit_res[0].values()):
                c_price = audit_res[1]
                sl = int(c_price * 0.95)
                tp1 = int(c_price + (c_price - sl) * 1.5)
                # Pyramiding Logic
                pyramid_price = int(c_price * 1.05)
                avg_new = int((c_price + pyramid_price) / 2)
                
                st.markdown(f"""
                <div class='stock-card'>
                    <h3>{t_sym} <span class='sector-badge'>{row['sector']}</span></h3>
                    <p style='margin:0; color:#10b981;'><b>🔥 ALPHA LEADER CONFIRMED</b></p>
                    <p>Price: {int(c_price)} | SL: {sl} | TP1: {tp1}</p>
                    <div class='pyramid-box'>
                        <b>📐 Pyramiding Plan:</b><br>
                        • Add Position at: {pyramid_price} (+5%)<br>
                        • New Average Price: {avg_new}<br>
                        • Action: Move SL to {int(c_price)} (Risk-Free)
                    </div>
                </div>
                """, unsafe_allow_html=True)

# =========================================================
# 🛡️ MODULE: PORTFOLIO & AUDIT (KOLOM CEK SAHAM)
# =========================================================
st.divider()
...
with col_audit:
    ...
    check_ticker = st.text_input("Masukkan Kode Saham (Contoh: BBRI):").upper()
...
with col_port:
    ...
    st.write("🛒 **Tambah/Hapus Pantauan**")
    t_manual = st.text_input("Kode Saham Portfolio:").upper()
    c1, c2 = st.columns(2)
    if c1.button("🛒 BELI"): st.success("Perintah ADD terkirim!")
    if c2.button("🗑️ JUAL"): st.error("Perintah DEL terkirim!")

st.caption("V45.0 OMNI-APEX | No modifications to UI/Theme as per Command.")