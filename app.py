...
st.set_page_config(page_title="V53.0 PRO-CONNECT", layout="wide", page_icon="💎")
...
# --- 🛰️ HEADER ---
st.markdown(f"<div class='status-card'><h1 style='margin:0; font-size: 28px; color:#ddd6fe;'>💎 V53.0 PRO-CONNECT</h1><p style='margin:0; opacity:0.8;'>Source: GoAPI Premium | World Champion Algorithm | Stability Locked</p></div>", unsafe_allow_html=True)
...
if is_market_open or bypass:
    ...
    # GoAPI: Fetch Trending/Prices for initial scan
    trending = get_goapi_data("trending")
    tickers = trending.get('results', [])[:25] # Ambil top 25 trending
    ...
    if tickers:
        ...
        st.warning("GoAPI sedang sinkronisasi data. Silakan tunggu.")
else:
    ...
    if st.button("🚀 Run Tactical Audit"):
        if tid_input:
            with st.spinner(f"Interogasi {tid_input}..."):
                ...
                else: st.error("Data GoAPI tidak ditemukan untuk ticker ini.")

with cb:
    st.subheader("🛡️ Portfolio & Buy