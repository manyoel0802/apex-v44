...
st.set_page_config(page_title="V47.4 PRESTIGE COMMANDER", layout="wide", page_icon="💎")

# --- TEMA VISUAL SUPREME (PROTECTED) ---
st.markdown("""
    <style>
...
def run_deep_audit(ticker, ihsg_ret):
    try:
        ...
        # 1. MTF & MINERVINI (WORLD CHAMPION LOGIC)
...
# --- 🛰️ HEADER ---
st.markdown(f"<div class='status-card'><h1 style='margin:0; font-size: 28px; color:#ddd6fe;'>🏆 V47.4 PRESTIGE COMMANDER</h1><p style='margin:0; opacity:0.8;'>Mode: Deep Scan Default | All Logic Integrated | High-Conviction Only</p></div>", unsafe_allow_html=True)

# --- 🎛️ SIDEBAR ---
with st.sidebar:
    st.header("⚙️ Command Center")
    cap = st.number_input("Capital (Rp)", value=1000000)
    # DEEP SCAN JADI DEFAULT (INDEX 1)
...
if is_market_open or bypass:
    ...
    try:
        ...
        for _, row in df_raw.iterrows():
            if mode == "Turbo (Fast)": checks, prc = {"Turbo Mode": True}, row['close']
            ...
            if checks and all(checks.values()):
                ...
                with cols[v_idx % 2]:
                    ...
                    """, unsafe_allow_html=True)
                v_idx += 1
    except: st.warning("Radar sedang memindai jagat bursa...")
else:
...
st.caption("V47.4 PRESTIGE | Deep Scan Default | Anti-Noise Mode Active.")